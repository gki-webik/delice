      ...
      <div class="blog-posts-list">
        <div v-for="post in posts" :key="post.id" class="blog-post-card">
          <div class="post-preview">
            <img v-if="post.image" :src="post.image" alt="Preview" />
...
export default {
  ...
  methods: {
    async fetchPosts() {
      try {
        const response = await fetch(this.apiBaseUrl + "/promotionsPosts", {
          credentials: "include",
        });
    ...
    openCreateModal() {
      alert(
        "Для создания акции необходимо обратиться к разработчику. Для связи: klicks.ru/my"
      );
    },
    editPost(post) {
